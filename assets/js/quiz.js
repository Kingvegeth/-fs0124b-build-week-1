let randomQuestions = []; // Array per memorizzare le domande ottenute dall'API

// Effettua la richiesta fetch e avvia il gioco una volta che le domande sono state ottenute
function fetchQuestionsAndStartGame() {
  fetch('https://opentdb.com/api.php?amount=10&category=18&difficulty=easy')
    ...
    .then(data => {
      randomQuestions = data.results; // Memorizza le domande ottenute dall'API
    ...
    .catch(error => {
      console.error('Si è verificato un errore nella richiesta fetch:', error);
    });
}
